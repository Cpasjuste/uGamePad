//
// Created by cpasjuste on 30/04/23.
//
...
bool format_fat12() {
    ...
    if (r != FR_OK) {
        printf("Error, f_mkfs failed with error code: %i\r\n", r);
        return false;
    }
    ...
    if (r != FR_OK) {
        printf("Error, f_mount failed with error code: %i\r\n", r);
        return false;
    }

    // Setting label
    printf("Setting disk label to: %s\r\n", DISK_LABEL);
    r = f_setlabel(DISK_LABEL);
    if (r != FR_OK) {
        printf("Error, f_setlabel failed with error code: %i\r\n", r);
        return false;
    }
...
bool check_fat12() {
    ...
    if (!fatfs.begin(&flash)) {
        printf("Error, failed to mount newly formatted filesystem!\r\n");
        return false;
    }
