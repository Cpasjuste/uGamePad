...
/* from samd21_host.c with fixes */
uint32_t USBHost::UHD_Pipe_Alloc(uint32_t ul_dev_addr, uint32_t ul_dev_ep, uint32_t ul_type, uint32_t ul_dir,
                                 uint32_t ul_maxsize, uint32_t ul_interval, uint32_t ul_nb_bank) {
    /* set pipe config */
...
uint32_t
USBHost::ctrlReq(uint32_t addr, uint32_t ep, uint8_t bmReqType, uint8_t bRequest, uint8_t wValLo, uint8_t wValHi,
...
/* rcode 0 if no errors. rcode 01-0f is relayed from dispatchPkt(). Rcode f0 means RCVDAVIRQ error,
   fe USB xfer timeout */
uint32_t
USBHost::inTransfer(uint32_t addr, uint32_t ep, uint16_t *nbytesptr, uint8_t *data, uint8_t bInterval /*= 0*/) {
    EpInfo *pep = NULL;
    uint32_t nak_limit = 0;
...
uint32_t
USBHost::InTransfer(EpInfo *pep, uint32_t nak_limit, uint16_t *nbytesptr, uint8_t *data, uint8_t bInterval /*= 0*/) {
    uint32_t rcode = 0;
    uint32_t pktsize = 0;
...
uint32_t USBHost::OutTransfer(EpInfo *pep, uint32_t nak_limit, uint32_t nbytes, uint8_t *data) {
    ...
    while (bytes_left) {
        ...
        memcpy(buf, data_p, bytes_tosend);
        UHD_Pipe_Write(pep->epAddr, bytes_tosend, buf); // data must be in RAM
...
uint32_t USBHost::AttemptConfig(uint32_t driver, uint32_t parent, uint32_t port, uint32_t lowspeed) {
    ...
    } else if (rcode != 0x00/*hrJERR*/ && retries <
                                          3) { // Some devices returns this when plugged in - trying to initialize the device again usually works
        delay(100);
        retries++;
        goto again;
    } else if (rcode)
...
uint32_t USBHost::getDevDescr(uint32_t addr, uint32_t ep, uint32_t nbytes, uint8_t *dataptr) {
    return (ctrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, 0x00, USB_DESCRIPTOR_DEVICE, 0x0000, nbytes,
                    nbytes, dataptr, 0));
}
//get configuration descriptor

uint32_t USBHost::getConfDescr(uint32_t addr, uint32_t ep, uint32_t nbytes, uint32_t conf, uint8_t *dataptr) {
    return (ctrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, conf, USB_DESCRIPTOR_CONFIGURATION, 0x0000,
                    nbytes, nbytes, dataptr, 0));
}
...
uint32_t USBHost::getConfDescr(uint32_t addr, uint32_t ep, uint32_t conf, USBReadParser *p) {
    ...
    return (ctrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, conf, USB_DESCRIPTOR_CONFIGURATION, 0x0000,
                    total, sizeof(buf), buf, p));
}

//get string descriptor

uint32_t
USBHost::getStrDescr(uint32_t addr, uint32_t ep, uint32_t nbytes, uint32_t index, uint32_t langid, uint8_t *dataptr) {
    return (ctrlReq(addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, index, USB_DESCRIPTOR_STRING, langid, nbytes,
                    nbytes, dataptr, 0));
}
//set address

uint32_t USBHost::setAddr(uint32_t oldaddr, uint32_t ep, uint32_t newaddr) {
    uint32_t rcode = ctrlReq(oldaddr, ep, bmREQ_SET, USB_REQUEST_SET_ADDRESS, newaddr, 0x00, 0x0000, 0x0000, 0x0000,
                             NULL, NULL);
...
uint32_t USBHost::setConf(uint32_t addr, uint32_t ep, uint32_t conf_value) {
    return (ctrlReq(addr, ep, bmREQ_SET, USB_REQUEST_SET_CONFIGURATION, conf_value, 0x00, 0x0000, 0x0000, 0x0000, NULL,
                    NULL));
}
